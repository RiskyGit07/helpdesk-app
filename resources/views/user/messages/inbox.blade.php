...
<div class="card">
        ...
        <div class="d-flex justify-content-between align-items-center w-100">
            <div>
                ...
                <div class="text-muted fs-7">
                    <i class="ki-outline ki-message-text-2"></i> Semua percakapan dengan Admin
                </div>
            </div>
    ...
    <div class="card-body pt-0">
        @php
            // Ambil semua tiket user yang memiliki balasan (respons)
            $tickets = App\Models\Ticket::where('user_id', Auth::id())
                ->whereHas('responses') // Hanya yang punya balasan
        ...
        @forelse($tickets as $ticket)
            @php
                ...
                $isLastMessageFromAdmin = $lastSender && $lastSender->user_type == 'admin';
            @endphp
                ...
                <div class="d-flex align-items-center py-4 border-bottom hover-bg-light rounded px-3 {{ $unreadCount > 0 ? 'bg-light-primary' : '' }}" style="transition: all 0.2s;">
                    ...
                    <div class="symbol symbol-50px me-4">
                        <div class="symbol-label bg-light-{{ $unreadCount > 0 ? 'primary' : 'secondary' }}">
                            <i class="ki-outline ki-message-text-2 fs-2x text-{{ $unreadCount > 0 ? 'primary' : 'gray-500' }}"></i>
                        </div>
                    </div>
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <div>
                                ...
                                <span class="badge badge-{{ $isLastMessageFromAdmin ? 'danger' : 'secondary' }} ms-2" style="font-size: 10px;">
                                    {{ $isLastMessageFromAdmin ? 'Admin' : 'User' }}
                                </span>
                            </div>
        ...
        @empty
            <div class="text-center py-10">
                ...
                <p class="text-muted">Belum ada percakapan dengan Admin</p>
                <a href="{{ route('user.tickets.create') }}" class="btn btn-primary">
                    <i class="ki-outline ki-add-files me-1"></i> Buat Pengaduan
                </a>
            </div>
        @endforelse
    </div>
